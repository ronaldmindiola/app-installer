...
const installPackage = async (app, packageId, spinner) => {
    spinner = ora(`Checking ${app}...`).start();
    spinner.succeed(`${app} not installed.`);

    // Verificar si la aplicación ya está instalada
    if (await isAppInstalled(packageId)) {
        spinner.succeed(`${app} ya está instalado1.`);
        return;
    }

    try {
        const { stdout, stderr } = await execAsync(`winget install --id=${packageId} -e`);
        spinner.succeed(`Instalación de ${app} completada.`);
